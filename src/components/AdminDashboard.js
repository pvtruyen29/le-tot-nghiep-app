            ...
            <details style={{ border: '1px solid #ccc', padding: '1.5rem', borderRadius: '8px', marginBottom: '2rem' }}>
                ...
                <form onSubmit={handleAddEvent} style={{ marginTop: '1rem' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1rem' }}>
                        ...
                        <input type="text" placeholder="Đơn vị tổ chức" value={organizer} onChange={(e) => setOrganizer(e.target.value)} />
                        <input type="text" placeholder="Nơi tổ chức" value={location} onChange={(e) => setLocation(e.target.value)} />
                        <div><label>Thời gian tổ chức:</label><input type="datetime-local" value={eventTime} onChange={(e) => setEventTime(e.target.value)} /></div>
                        <div><label>Bắt đầu đăng ký:</label><input type="datetime-local" value={startTime} onChange={(e) => setStartTime(e.target.value)} /></div>
                        <div><label>Hạn chót đăng ký:</label><input type="datetime-local" value={endTime} onChange={(e) => setEndTime(e.target.value)} /></div>
                    </div>
                    <textarea placeholder="Ghi chú, lưu ý..." value={notes} onChange={(e) => setNotes(e.target.value)} style={{ width: 'calc(100% - 16px)', marginTop: '1rem', minHeight: '80px', padding: '8px' }}></textarea>
